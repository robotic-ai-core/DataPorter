...
class FastLeRobotDataset(LeRobotDataset):
    ...
    When ``cache_frames=True``, also overrides ``_query_videos`` to decode
    each episode's video once and serve frames from an in-memory LRU cache
    bounded by ``cache_budget_gb``.

    Performance improvement:
        - hf_dataset.select([5,6,7,8,9]): ~2.6ms (original)
        - hf_dataset[5:10] (slice): ~0.2ms (10x faster)
        - Loop of hf_dataset[i]: ~0.5ms (5x faster, for non-contiguous)

    Example:
        >>> dataset = FastLeRobotDataset(
        ...     "lerobot/pusht",
        ...     delta_timestamps={"observation.image": [0.0, 0.1, 0.2]},
        ...     cache_frames=True,
        ...     cache_budget_gb=2.0,
        ... )
        >>> item = dataset[0]  # 10x faster than LeRobotDataset
    """

    def __init__(self, *args, cache_frames: bool = False,
                 cache_budget_gb: float = 2.0, **kwargs):
        super().__init__(*args, **kwargs)
        self._cache_frames = cache_frames
        self._cache_budget_bytes = int(cache_budget_gb * 1024**3)
        # LRU cache: OrderedDict keyed by (ep_idx, vid_key) -> uint8 tensor
    ...
    def _fast_hf_access(self, indices: list[int]) -> dict:
        ...
        Args:
            indices: List of row indices to fetch.

        Returns:
            Dict with column data, same format as hf_dataset.select().
        """
        if not indices:
            return {}
        ...
        if self._is_contiguous(unique_indices):
            ...
            # Handle repeated indices (e.g., at episode boundaries)
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        ...
        Overrides parent to avoid slow hf_dataset.select().

        Args:
            query_indices: Dict mapping keys to lists of indices.

        Returns:
            Dict mapping keys to stacked tensors.
        """
        result = {}
    ...
    def _get_query_timestamps(
        ...
        Returns:
            Dict mapping video keys to lists of timestamps.
        """
        query_timestamps = {}
    ...
    def _query_videos(
        ...
        for vid_key, query_ts in query_timestamps.items():
            ...
            if cache_key in self._frame_cache:
                self._frame_cache.move_to_end(cache_key)  # LRU refresh
            else:
    ...
    def _cache_episode_frames(self, ep_idx: int, vid_key: str) -> None:
        ...
        # decode_video_frames may return [1, T, C, H, W] or [T, C, H, W]
        if all_frames.dim() == 5:
